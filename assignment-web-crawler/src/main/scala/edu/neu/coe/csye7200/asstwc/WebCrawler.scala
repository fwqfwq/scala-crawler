...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 9 points.
    for(f <- getURLContent(u); g <- MonadOps.asFuture(getLinks(f))) yield g // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val ts = for(u <- us2) yield wget(u)
    val list = for(t <- ts) yield MonadOps.sequence(t) //list of url
